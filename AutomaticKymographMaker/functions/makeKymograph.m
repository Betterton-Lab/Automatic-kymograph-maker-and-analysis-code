function Kymograph = makeKymograph(movie4D, pos_cut7_pk, first_frame, last_frame, time_step, channelType)
    ...
    for frame_number = first_frame:last_frame
        ...
        end_pos2= cut7_pos1 + (cut7_pos2-cut7_pos1)*ratio7_to_max(frame_number)...
            +(cut7_pos2-cut7_pos1)/norm(cut7_pos2-cut7_pos1)*AbsEndExt;
    ...
    title_name = strcat("Kymograph of ", channelType, " intensity alone cut7 peak to peak");
	title(title_name);
	...
	plot(cut7_pk_pos_on_kymograph(first_frame:last_frame, 1)*interp_step+1, times, 'r.', 'LineWidth',1);
